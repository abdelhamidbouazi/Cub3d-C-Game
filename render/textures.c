...
/*   Updated: 2023/07/10 09:21:46 by abouazi          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void texture(t_data *data)
{
	int	h;
	int	w;
	data->image.img_window = mlx_xpm_file_to_image(data->mlx, data->map->so, &w, &h);
	...
	data->image.img_window = mlx_xpm_file_to_image(data->mlx, data->map->ea, &w, &h);
	if (!data->image.img_window)
		exit(0);
	data->image.no = (int *)mlx_get_data_addr(data->image.img_window,
			...
			&data->image.line_len,
			&data->image.endian);
	data->image.img_window = mlx_xpm_file_to_image(data->mlx, data->map->no, &w, &h);
	if (!data->image.img_window)
		exit(0);
	data->image.we = (int *)mlx_get_data_addr(data->image.img_window,
			...
			&data->image.line_len,
			&data->image.endian);
	data->image.img_window = mlx_xpm_file_to_image(data->mlx,data->map->we, &w, &h);
	if (!data->image.img_window)
		exit(0);
